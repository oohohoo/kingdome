...
$(".fs-nav-item, .hover-opacity, .nav-hoverinfo, .kupole-info").each(function(i, el) {
...
/*   gsap.utils.toArray('.foe').forEach((el, i) => {
  ...
  ScrollTrigger.batch('.card', {
    ...
    onEnter: batch => {
      batch.forEach((card, index ) => gsap.to(card.children, {y:0, autoAlpha:1, stagger: 0.025, delay: index * 0.3}))
    },
    once: true
  });
